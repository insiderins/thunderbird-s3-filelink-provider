...
async function signAwsV4(fields) {
    ...
    return `AWS4-HMAC-SHA256 Credential=${accessKey}/${credential}, SignedHeaders=${signedHeaders}, Signature=${signature}`;
}
...
async function uploadFile(account, upload, data) {
    ...
    const filePath = `${getAccountPrefix(
        account
    )}/${awsDate}-SHA256-${fileSha256}/${encodeURIRFC3986(name)}`;
    const contentLength = data.size || data.byteLength || 0;

    console.log(
        `Uploading file: ${name}, size: ${contentLength} bytes, SHA256: ${fileSha256}`
    );
...
browser.cloudFile.onFileUpload.addListener(
    async (account, { id, name, data }) => {
        try {
            console.log(`Starting upload for file: ${name}, ID: ${id}`);
            ...
            const { response, url } = await uploadFile(
                accountInfo,
                upload,
                data
            );
            ...
            if (response.status === 200 || response.status === 204) {
                console.log(`Upload success for ${name}:`, response.status);
                return { url };
            }
            ...
            console.error(
                `Upload failed for ${name} (HTTP ${response.status}):`,
                errorText
            );
